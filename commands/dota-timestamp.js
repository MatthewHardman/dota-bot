...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const formattedEndTime = `<t:${endTime}:F>`;

    const message = await interaction.reply({
      content: `Hello <@&${role.id}>, ${interaction.user.username} would like to play with a stack size of ${stackSize} and is willing to wait ${timeOutInMin} minutes (${formattedEndTime})! Please react if you'd like to be pinged when a stack forms. If you initiated the command, I have reacted for you. (${formattedStartTime})`,
      fetchReply: true,
    });
